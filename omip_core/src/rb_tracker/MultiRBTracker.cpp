...
{
    // Was pcl_conversions::toPCL(ros::Time(measurement_timestamp_ns/1e9 +
    // loop_period_ns/1e9)) — PCL's own header.stamp is microseconds-since-
    // epoch by PCL's own convention (independent of ROS); this is the same
    // arithmetic (seconds -> microseconds) without the ros::Time round trip.
    this->_predicted_measurement->header.stamp =
        static_cast<std::uint64_t>((this->_measurement_timestamp_ns + this->_loop_period_ns) / 1000.0);
...
std::vector<Feature::Id> MultiRBTracker::estimateBestPredictionsAndSupportingFeatures()
{
    ...
    for (std::vector<RBFilter::Ptr>::iterator filter_it = this->_kalman_filters.begin(); filter_it != this->_kalman_filters.end();)
    {
                ...
                && *filter_it != this->_static_environment_filter )
        {
            ...
            OMIP_ERROR_NAMED( "MultiRBTracker.estimateBestPredictionsAndSupportingFeatures",
...
void MultiRBTracker::correctState()
{
    ...
    OMIP_INFO_NAMED("MultiRBTracker::correctState","Moving bodies: %3d, free features: %3d", (int)this->_kalman_filters.size(), (int)really_free_feat_ids.size());
    ...
    if(this->_kalman_filters.size() < (size_t)this->_max_num_rb)
    {
...
void MultiRBTracker::ReflectState()
{
    ...
    std::vector<TwistWithCovariance::Ptr> tracked_poses_twist_with_cov = this->getPosesECWithCovariance();
    std::vector<TwistWithCovariance::Ptr> tracked_vels_twist_with_cov = this->getVelocitiesWithCovariance();
    ...
    for (size_t poses_idx = 0; poses_idx < tracked_poses_twist_with_cov.size(); poses_idx++)
    {
        ...
        RigidBodyPoseAndVel rbpose_and_vel;
        ...
        rbpose_and_vel.centroid = centroids.at(poses_idx);
...
void MultiRBTracker::processMeasurementFromShapeTracker(const ShapeTrackerStates &meas_from_st)
{
    for(size_t idx = 0; idx < meas_from_st.shape_tracker_states.size(); idx++)
    {
        BOOST_FOREACH(RBFilter::Ptr filter, this->_kalman_filters)
        {
            if (filter->getId() == meas_from_st.shape_tracker_states.at(idx).rb_id )
            {
                ...
                if((meas_from_st.shape_tracker_states.at(idx).num_points_of_model > _min_num_points_in_segment) &&
                        (meas_from_st.shape_tracker_states.at(idx).num_points_of_current_pc > _min_num_points_in_segment) &&
                ...
                {
                    OMIP_ERROR_STREAM("Integrating shape-based tracked pose RB " << meas_from_st.shape_tracker_states.at(idx).rb_id);
                    filter->integrateShapeBasedPose(meas_from_st.shape_tracker_states.at(idx).pose_wc, this->_measurement_timestamp_ns - meas_from_st.timestamp_ns );
                }else{
                    OMIP_ERROR_STREAM("Ignoring shape-based tracked pose RB " << meas_from_st.shape_tracker_states.at(idx).rb_id);
                }
            }
        }
    }
...
void MultiRBTracker::TryToCreateNewFilter(const std::vector<Feature::Id>& really_free_feat_ids)
{
    ...
    if(really_free_feat_ids.size() > (size_t)this->_min_num_feats_for_new_rb)
    {
        ...
        if(index > this->_min_num_feats_for_new_rb)
        {
            ...
            {
                OMIP_INFO_STREAM_NAMED("RBMTracker._createNewFilters", "Not enough Consensus to create a new Filter with the free Features ("<<
...
std::vector<PoseWithCovariance::Ptr> MultiRBTracker::getPosesWithCovariance() const
{
    std::vector<PoseWithCovariance::Ptr> return_filter_results;
    BOOST_FOREACH(RBFilter::Ptr filter, this->_kalman_filters)
    {
...
std::vector<TwistWithCovariance::Ptr> MultiRBTracker::getPosesECWithCovariance() const
{
    std::vector<TwistWithCovariance::Ptr> return_filter_results;
    BOOST_FOREACH(RBFilter::Ptr filter, this->_kalman_filters)
    {
...
std::vector<TwistWithCovariance::Ptr> MultiRBTracker::getVelocitiesWithCovariance() const
{
    std::vector<TwistWithCovariance::Ptr> return_filter_results;
    BOOST_FOREACH(RBFilter::Ptr filter, this->_kalman_filters)
    {
...
std::vector<Eigen::Vector3d> MultiRBTracker::getCentroids() const
{
    ...
    for (; filter_it != this->_kalman_filters.end(); filter_it++)
    {
        ...
        if((*filter_it)->getTrajectory().size() < (size_t)this->_min_num_frames_for_new_rb)
        {
...
{
    BOOST_FOREACH(RigidBodyPoseAndVel predicted_pose_and_vel, predicted_state.rb_poses_and_vels)
    {
        BOOST_FOREACH(RBFilter::Ptr filter, this->_kalman_filters)
        {
