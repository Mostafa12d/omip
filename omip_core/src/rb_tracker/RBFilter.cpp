...
#include "omip_core/rb_tracker/RBFilter.h"
...
void RBFilter::predictState(double time_interval_ns)
{
    ...
    {
        OMIP_ERROR_STREAM("Time interval negative!");
    ...
    Twistd belief_delta_pose_ec = (time_interval_ns/1e9)*_velocity;
    _predicted_pose_vh = se3Exp(belief_delta_pose_ec, 1e-12).matrix()*_pose;
    ...
    this->_predicted_velocity_bh = Twistd(0,0,0,0,0,0);
...
void RBFilter::correctState()
{
    ...
    if(num_supporting_feats > this->_min_num_supporting_feats_to_correct)
    {
        ...
        Twistd pose_update_ec(pose_update[3], pose_update[4], pose_update[5], pose_update[0], pose_update[1], pose_update[2]);
        _pose = se3Exp(pose_update_ec, 1e-12).matrix()*_predicted_pose;
    }else{
        OMIP_ERROR_STREAM_NAMED("RBFilter::correctState","RB" << this->_id << ": using predicted state as corrected state!");
...
void RBFilter::estimateBestPredictionAndSupportingFeatures()
{
    ...
    case 0:
    {
        OMIP_INFO_STREAM("RB" << _id<<" Using velocity prediction");
    ...
    case 1:
    {
        OMIP_INFO_STREAM("RB" << _id<<" Using brake prediction");
    ...
    case 2:
    {
        OMIP_ERROR_STREAM_NAMED("RBFilter.estimateBestPredictionAndSupportingFeatures",
...
void RBFilter::setPredictedState(RigidBodyPoseAndVel hypothesis)
{
    this->_use_predicted_state_from_kh = true;

    // hypothesis.pose_wc.twist / velocity_wc.twist are already omip::Twistd
    // (unlike the original geometry_msgs::Twist, which needed field-by-field
    // reconstruction into Eigen::Twistd) — direct copy.
    Twistd predicted_pose_kh_ec = hypothesis.pose_wc.twist;
    this->_predicted_pose_kh = se3Exp(predicted_pose_kh_ec, 1e-12).matrix();

    this->_predicted_velocity_kh = hypothesis.velocity_wc.twist;

    this->_predicted_pose_cov_kh = hypothesis.pose_wc.covariance;
    this->_predicted_velocity_cov_kh = hypothesis.velocity_wc.covariance;
}

void RBFilter::integrateShapeBasedPose(const TwistWithCovariance twist_refinement, double pose_time_elapsed_ns)
{
    Twistd shape_based_pose_ec = twist_refinement.twist;

    if(pose_time_elapsed_ns < this->_loop_period_ns || _supporting_features_ids.size() < (size_t)_min_num_supporting_feats_to_correct)
    {
        OMIP_INFO_STREAM_NAMED("RBFilter.integrateShapeBasedPose", "Using shape-based pose (time elapsed=" << pose_time_elapsed_ns/1e9 <<
                              " s, loop_period="<< _loop_period_ns/1e9 << " s)");

        this->_pose = se3Exp(shape_based_pose_ec, 1e-12).matrix();
        ...
        OMIP_INFO_STREAM_NAMED("RBFilter.integrateShapeBasedPose", "Shape-based pose is too old (time elapsed=" << pose_time_elapsed_ns/1e9 <<
...
Twistd RBFilter::getVelocity() const
{
    return _velocity;
}
...
PoseWithCovariance::Ptr RBFilter::getPoseWithCovariance() const
{
    PoseWithCovariance::Ptr pose_wc_ptr(new PoseWithCovariance());
    pose_wc_ptr->pose = Eigen::Isometry3d(_pose);
    pose_wc_ptr->covariance = _pose_covariance;
    return pose_wc_ptr;
}

TwistWithCovariance::Ptr RBFilter::getPoseECWithCovariance() const
{
    TwistWithCovariance::Ptr pose_wc_ptr(new TwistWithCovariance());
    Twistd pose_ec;
    TransformMatrix2Twist(_pose, pose_ec);

    pose_wc_ptr->twist = pose_ec;
    pose_wc_ptr->covariance = _pose_covariance;

    return pose_wc_ptr;
}

TwistWithCovariance::Ptr RBFilter::getVelocityWithCovariance() const
{
    TwistWithCovariance::Ptr vel_t_wc_ptr(new TwistWithCovariance());
    vel_t_wc_ptr->twist = _velocity;
    vel_t_wc_ptr->covariance = _velocity_covariance;
...
FeatureCloudPCLwc RBFilter::getPredictedMeasurement(PredictionHypothesis hypothesis)
{
    ...
    case BASED_ON_BRAKING_EVENT:
        ...
        break;
    default:
        OMIP_ERROR_STREAM_NAMED("RBFilter::getPredictedMeasurement", "Wrong hypothesis. Value " << hypothesis);
        break;
    }
