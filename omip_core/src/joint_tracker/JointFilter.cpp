#include "omip_core/joint_tracker/JointFilter.h"
...
void JointFilter::setInitialMeasurement(const joint_measurement_t &initial_measurement,
                                        const Twistd& rrb_pose_at_srb_birth_in_sf,
                                        const Twistd& srb_pose_at_srb_birth_in_sf)
{
    ...
    this->_rrb_current_pose_in_sf = initial_measurement.first.pose_wc.twist;
    ...
    this->_rrb_current_vel_in_sf = initial_measurement.first.velocity_wc.twist;
    ...
    this->_srb_current_pose_in_sf = initial_measurement.second.pose_wc.twist;
    ...
    // TwistCoord({sfbf}|SRB,{sf}|S,[sf])(t)
    this->_srb_current_vel_in_sf = initial_measurement.second.velocity_wc.twist;

    // Extract covariance of the reference and the second RB wrt sensor frame
    this->_rrb_pose_cov_in_sf = initial_measurement.first.pose_wc.covariance;
    this->_rrb_vel_cov_in_sf = initial_measurement.first.velocity_wc.covariance;
    this->_srb_pose_cov_in_sf = initial_measurement.second.pose_wc.covariance;
    ...
    Eigen::Isometry3d T_sf_srbf_t0 = se3Exp(srb_pose_at_srb_birth_in_sf, 1e-20);
    Eigen::Isometry3d T_sf_rrbf_t0 = se3Exp(rrb_pose_at_srb_birth_in_sf, 1e-20);
    Eigen::Isometry3d T_rrbf_sf_t0 = T_sf_rrbf_t0.inverse();
    Eigen::Isometry3d T_rrbf_srbf_t0 = T_rrbf_sf_t0 * T_sf_srbf_t0;
    this->_srb_initial_pose_in_rrbf = se3Log(T_rrbf_srbf_t0, 1.0e-20);
    ...
    Eigen::Isometry3d T_sf_rrbf_t = se3Exp(this->_rrb_current_pose_in_sf, 1e-20);
    Eigen::Isometry3d T_sf_srbf_t = se3Exp(this->_srb_current_pose_in_sf, 1e-20);
    Eigen::Isometry3d T_rrbf_sf_t = T_sf_rrbf_t.inverse();
    Eigen::Isometry3d T_rrbf_srbf_t = T_rrbf_sf_t * T_sf_srbf_t;
    this->_srb_current_pose_in_rrbf = se3Log(T_rrbf_srbf_t, 1.0e-20);
    ...
    this->_current_delta_pose_in_rrbf = se3Log(se3Exp(_srb_current_pose_in_rrbf, 1e-12) * se3Exp(_srb_initial_pose_in_rrbf, 1e-12).inverse(), 1e-12);
    ...
    {
        this->_rrb_centroid_in_sf = initial_measurement.first.centroid;
    }

    // Extract centroid of the second RB in sensor frame
    this->_srb_centroid_in_sf = initial_measurement.second.centroid;
}
...
void JointFilter::_initVariables(JointCombinedFilterId joint_id)
{
    ...
    this->_rrb_current_pose_in_sf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_rrb_previous_pose_in_sf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_rrb_current_vel_in_sf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_current_pose_in_sf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_current_vel_in_sf = Twistd(0.,0.,0.,0.,0.,0.);

    this->_srb_initial_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_current_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_previous_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_predicted_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_srb_previous_predicted_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_current_delta_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
    this->_previous_delta_pose_in_rrbf = Twistd(0.,0.,0.,0.,0.,0.);
...
void JointFilter::setMeasurement(joint_measurement_t acquired_measurement, const double &measurement_timestamp_ns)
{
    ...
    this->_rrb_current_pose_in_sf = acquired_measurement.first.pose_wc.twist;
    // Extract velocity of the reference RB in sensor frame
    this->_rrb_current_vel_in_sf = acquired_measurement.first.velocity_wc.twist;
    // Extract pose of the second RB in sensor frame
    this->_srb_current_pose_in_sf = acquired_measurement.second.pose_wc.twist;
    // Extract velocity of the reference RB in sensor frame
    this->_srb_current_vel_in_sf = acquired_measurement.second.velocity_wc.twist;
    // Extract covariance of the reference and the second RB wrt sensor frame
    this->_rrb_pose_cov_in_sf = acquired_measurement.first.pose_wc.covariance;
    this->_rrb_vel_cov_in_sf = acquired_measurement.first.velocity_wc.covariance;
    this->_srb_pose_cov_in_sf = acquired_measurement.second.pose_wc.covariance;

    Eigen::Isometry3d T_sf_rrbf = se3Exp(this->_rrb_current_pose_in_sf, 1.0e-20);
    Eigen::Isometry3d T_sf_srbf = se3Exp(this->_srb_current_pose_in_sf, 1.0e-20);
    Eigen::Isometry3d T_rrbf_sf = T_sf_rrbf.inverse();
    Eigen::Isometry3d T_rrbf_srbf = T_rrbf_sf * T_sf_srbf;
    this->_srb_current_pose_in_rrbf = se3Log(T_rrbf_srbf, 1e-20);
    ...
    Eigen::Isometry3d delta_displ = T_rrbf_srbf * (T_rrbf_srbf_t0.inverse());

    this->_current_delta_pose_in_rrbf = se3Log(delta_displ, 1.0e-20);
    ...
    {
        this->_rrb_centroid_in_sf = acquired_measurement.first.centroid;
    }

    // Extract centroid of the second RB in sensor frame
    this->_srb_centroid_in_sf = acquired_measurement.second.centroid;
}

Twistd JointFilter::getPredictedMeasurement() const
{
...
{
    // Originally published a geometry_msgs::PoseWithCovarianceStamped over
    // ROS for visualization, gated behind `#ifdef PUBLISH_PREDICTED_POSE_AS_PWC`
    // — a macro permanently commented out in the original source, so this
    // was dead code by construction. Dropped, see JointFilter.h header note.
}
