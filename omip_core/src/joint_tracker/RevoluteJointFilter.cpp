#include "omip_core/joint_tracker/RevoluteJointFilter.h"
...
void RevoluteJointFilter::estimateMeasurementHistoryLikelihood()
{
    ...
    for (size_t sample_idx = 0; sample_idx < amount_samples; sample_idx++)
    {
        ...
        Eigen::Isometry3d rb2_last_delta_relative_displ_rev_hyp = se3Exp(Twistd( rev_joint_rotation.x(),
                                                                                    rev_joint_rotation.y(),
                                                                                    rev_joint_rotation.z(),
                                                                                    rev_joint_translation.x(),
                                                                                    rev_joint_translation.y(),
                                                                                    rev_joint_translation.z()), 1e-12);
        Eigen::Vector3d rb2_last_delta_relative_translation_rev_hyp = rb2_last_delta_relative_displ_rev_hyp.translation();
        Eigen::Quaterniond rb2_last_delta_relative_rotation_rev_hyp = Eigen::Quaterniond(rb2_last_delta_relative_displ_rev_hyp.linear());
...
void RevoluteJointFilter::estimateUnnormalizedModelProbability()
{
    ...
    Eigen::Vector4d point1_in_sf_homo = se3Exp(_rrb_current_pose_in_sf, 1e-12).matrix()*point1_in_rrbf_homo;
    ...
    Eigen::Vector4d point2_in_sf_homo = se3Exp(_rrb_current_pose_in_sf, 1e-12).matrix()*point2_in_rrbf_homo;
...
TwistWithCovariance RevoluteJointFilter::getPredictedSRBDeltaPoseWithCovInSensorFrame()
{
...
TwistWithCovariance RevoluteJointFilter::getPredictedSRBVelocityWithCovInSensorFrame()
{
    Eigen::Matrix<double, 6, 6> adjoint;
    computeAdjoint(this->_rrb_current_pose_in_sf, adjoint);
...
TwistWithCovariance RevoluteJointFilter::getPredictedSRBPoseWithCovInSensorFrame()
{
...
std::vector<DebugGeometry> RevoluteJointFilter::getJointDebugGeometryInRRBFrame() const
{
    ...
    std::vector<DebugGeometry> revolute_markers;
    ...
    axis_orientation_marker.color = Eigen::Vector4d(1.f, 0.f, 0.f, 1.f);
    Eigen::Vector3d point_on_rot_axis_1 = rev_joint_posi_in_ref_rb - this->_joint_state * rev_joint_ori_in_ref_rb;
    axis_orientation_marker.points.push_back(point_on_rot_axis_1);
    Eigen::Vector3d point_on_rot_axis_2 = rev_joint_posi_in_ref_rb + this->_joint_state * rev_joint_ori_in_ref_rb;
    axis_orientation_marker.points.push_back(point_on_rot_axis_2);
    revolute_markers.push_back(axis_orientation_marker);

    // AXIS MARKER 2 -> Proportional to the joint state///////////////////////////////////////////////////////////////////////////////////////////////////////////
    DebugGeometry axis_orientation_markerb;
    axis_orientation_markerb.ns = "kinematic_structure";
    axis_orientation_markerb.action = DebugGeometryAction::Add;
    axis_orientation_markerb.type = DebugGeometryType::Arrow;
    axis_orientation_markerb.id = 3 * this->_joint_id + 1;
    axis_orientation_markerb.scale = Eigen::Vector3d(JOINT_AXIS_MARKER_RADIUS, 0.f, 0.f);
    axis_orientation_markerb.color = Eigen::Vector4d(1.f, 0.f, 0.f, 1.f);
    Eigen::Vector3d point_on_rot_axis_1b = rev_joint_posi_in_ref_rb - 100 * rev_joint_ori_in_ref_rb;
    axis_orientation_markerb.points.push_back(point_on_rot_axis_1b);
    Eigen::Vector3d point_on_rot_axis_2b = rev_joint_posi_in_ref_rb  + 100 * rev_joint_ori_in_ref_rb;
    axis_orientation_markerb.points.push_back(point_on_rot_axis_2b);
    ...
    axis_orientation_markerb.type = DebugGeometryType::TextViewFacing;
    ...
    axis_orientation_markerb.position = rev_joint_posi_in_ref_rb;
    axis_orientation_markerb.orientation = Eigen::Quaterniond::Identity();
    ...
    axis_position_uncertainty_marker.position = rev_joint_posi_in_ref_rb;
    ...
    axis_position_uncertainty_marker.action = DebugGeometryAction::Add;
    ...
    axis_position_uncertainty_marker.scale = Eigen::Vector3d(this->_uncertainty_joint_position(0,0), //Using start and end points, scale.x is the radius of the array body
                                                              this->_uncertainty_joint_position(1,1), //Using start and end points, scale.y is the radius of the array head
                                                              this->_uncertainty_joint_position(2,2)); //Using start and end points, scale.y is the radius of the array head
    axis_position_uncertainty_marker.color = Eigen::Vector4d(1.0, 0.0, 0.0, 0.3);
    revolute_markers.push_back(axis_position_uncertainty_marker);

    DebugGeometry rev_axis_unc_cone1;
    rev_axis_unc_cone1.type = DebugGeometryType::MeshResource;
    rev_axis_unc_cone1.action = DebugGeometryAction::Add;
    rev_axis_unc_cone1.mesh_resource = "package://joint_tracker/meshes/cone.stl";
    rev_axis_unc_cone1.position = rev_joint_posi_in_ref_rb;
    ...
    rev_axis_unc_cone1.orientation = ori_quat_final_ellipse;
    rev_axis_unc_cone1.ns = "kinematic_structure_uncertainty";
    rev_axis_unc_cone1.id = 3 * this->_joint_id + 1;
    rev_axis_unc_cone1.color = Eigen::Vector4d(1.0, 0.0, 0.0, 0.4);
    ...
    rev_axis_unc_cone1.scale = Eigen::Vector3d(major_axis_length / (M_PI / 6.0), minor_axis_length / (M_PI / 6.0), 1.);
    ...
    rev_axis_unc_cone1.orientation = ori_quat_neg_final_ellipse;
    rev_axis_unc_cone1.scale = Eigen::Vector3d(major_axis_length / (M_PI / 6.0), minor_axis_length / (M_PI / 6.0), 1.);
