...
#include "omip_core/feature_tracker/FeatureTracker.h"
...
namespace
{
// Replaces ros::Time::now(): wall-clock time in seconds since epoch. Used
// only for the debug-image getters' timestamps and the BETA "attention to
// motion" gate — see PORTING_NOTES.md Phase 2 open question re: whether
// this gate should instead be driven by simulation time once used from the
// MuJoCo orchestrator.
...
double wallClockNowNs()
{
    return wallClockNowSeconds() * 1e9;
}

// Replaces cv_bridge::cvtColor(img, "mono8"): converts a StampedImage to
// mono8, given the small set of encodings the original RGB-D pipeline
// actually produces. Not a ROS-only feature — cv_bridge's cvtColor is
// itself just a thin wrapper around cv::cvtColor with an encoding-string
// lookup table; this reimplements only the entries this codebase uses.
StampedImage::Ptr toMono8(const StampedImage::Ptr& img)
{
    StampedImage::Ptr out(new StampedImage());
    out->encoding = "mono8";
    out->timestamp_ns = img->timestamp_ns;
    if (img->encoding == "mono8")
    {
        out->image = img->image.clone();
    }
    else if (img->encoding == "bgr8" || img->encoding == "bgra8")
    {
        cv::cvtColor(img->image, out->image, img->encoding == "bgr8" ? cv::COLOR_BGR2GRAY : cv::COLOR_BGRA2GRAY);
    }
    else if (img->encoding == "rgb8" || img->encoding == "rgba8")
    {
        cv::cvtColor(img->image, out->image, img->encoding == "rgb8" ? cv::COLOR_RGB2GRAY : cv::COLOR_RGBA2GRAY);
    }
    else
    {
        OMIP_ERROR_STREAM_NAMED("PointFeatureTracker.toMono8", "Unsupported RGB image encoding: " << img->encoding);
        out->image = img->image.clone();
    }
    return out;
}
...
PointFeatureTracker::PointFeatureTracker(double loop_period_ns, const FeatureTrackerConfig& config, bool using_pc, std::string ft_ns) :
...
void PointFeatureTracker::predictMeasurement()
{
    ...
    if (!this->_first_frame && this->_camera_info_msg_rcvd && this->_predicted_states.size() > 1 && this->_predicted_states[1] &&
            (double)(this->_predicted_states[1]->header.stamp) * 1000.0 - this->_measurement_timestamp_ns < this->_loop_period_ns/2.0)
    {
        ...
        cv::Point predicted_feat_2d_cv_int = cv::Point(0, 0);
        cv::Point2f predicted_feat_2d_cv_f = cv::Point2f(0.f, 0.f);
...
void PointFeatureTracker::setOcclusionMaskImg(const cv::Mat& occ_mask_img)
{
    this->_occlusion_msk_img_ptr = StampedImage::Ptr(new StampedImage());
    this->_occlusion_msk_img_ptr->image = occ_mask_img;
...
void PointFeatureTracker::_ProcessDepthImg()
{
    ...
    {
        // First time
        if(this->_previous_time_of_motion_detection_s == 0.0)
        {
            this->_previous_depth_mat = this->_depth_img_ptr->image;
            this->_previous_time_of_motion_detection_s = wallClockNowSeconds();
        }
        else
        {
            if((wallClockNowSeconds() - this->_previous_time_of_motion_detection_s) > this->_min_time_to_detect_motion)
            {
...
void PointFeatureTracker::_ProcessRGBImg()
{
    ...
    this->_current_bw_img_ptr = toMono8(this->_rgb_img_ptr);
    _measurement_timestamp_ns = this->_rgb_img_ptr->timestamp_ns;
}
...
void PointFeatureTracker::setCameraInfoMsg(const CameraIntrinsics& camera_info)
{
    this->_camera_info_ptr = CameraIntrinsics::Ptr(new CameraIntrinsics(camera_info));
    ...
    {
        OMIP_ERROR_STREAM_NAMED("PointFeatureTracker.setCameraInfoMsg", "Resizing predicted features mask!");
...
StampedImage::Ptr PointFeatureTracker::getTrackedFeaturesImg()
{
    if(this->_publishing_tracked_feats_img && this->_rgb_img_ptr)
    {
        ...
        this->_tracked_features_img->timestamp_ns = wallClockNowNs();

        for (size_t tracked_feats_idx = 0; tracked_feats_idx < this->_corrected_measurement.size(); tracked_feats_idx++)
        {
            cv::Point p = cv::Point(cvRound(this->_corrected_measurement[tracked_feats_idx].x),
...
StampedImage::Ptr PointFeatureTracker::getRGBImg()
{
    return this->_rgb_img_ptr;
}

StampedImage::Ptr PointFeatureTracker::getDepthImg()
{
    return this->_depth_img_ptr;
}

StampedImage::Ptr PointFeatureTracker::getTrackedFeaturesWithPredictionMaskImg()
{
    if(this->_publishing_tracked_feats_with_pred_msk_img)
    {
        this->_tracked_features_with_pred_msk_img->image.setTo(0);
        this->_tracked_features_with_pred_msk_img->timestamp_ns = wallClockNowNs();
...
StampedImage::Ptr PointFeatureTracker::getDepthEdgesImg()
{
    this->_depth_edges_img->timestamp_ns = wallClockNowNs();
    this->_depth_edges_img->encoding = "mono8";
    this->_depth_edges_img->image = this->_canny_edges_mat;
    return  this->_depth_edges_img;
}

StampedImage::Ptr PointFeatureTracker::getPredictingMaskImg()
{
    this->_predicting_mask_img->timestamp_ns = wallClockNowNs();
...
StampedImage::Ptr PointFeatureTracker::getTrackingMaskImg()
{
    this->_tracking_mask_img->timestamp_ns = wallClockNowNs();
...
StampedImage::Ptr PointFeatureTracker::getDetectingMaskImg()
{
    this->_detecting_mask_img->timestamp_ns = wallClockNowNs();
...
StampedImage::Ptr PointFeatureTracker::getPredictedAndLastFeaturesImg()
{
    if(this->_publishing_predicted_and_past_feats_img && this->_rgb_img_ptr)
    {
        ...
        this->_predicted_and_previous_features_img->timestamp_ns = wallClockNowNs();

        //
        for (size_t tracked_feats_idx = 0;tracked_feats_idx < this->_predicted_measurements[0].size(); tracked_feats_idx++)
        {
            cv::Point previous = cv::Point(cvRound(this->_predicted_measurements[0][tracked_feats_idx].x),
                    cvRound(this->_predicted_measurements[0][tracked_feats_idx].y));
            cv::Point predicted = cv::Point(cvRound(this->_predicted_measurements[1][tracked_feats_idx].x),
...
{
    this->_max_allowed_depth = _config.max_distance;
    this->_min_allowed_depth = _config.min_distance;
    this->_number_of_features = _config.number_features;
    this->_min_number_of_features = _config.min_number_features;
    this->_min_feat_quality = _config.min_feat_quality;
    this->_max_interframe_jump = _config.max_interframe_jump;

    this->_publishing_tracked_feats_with_pred_msk_img = _config.pub_tracked_feats_with_pred_mask_img;
    this->_publishing_tracked_feats_img = _config.pub_tracked_feats_img;
    this->_publishing_predicted_and_past_feats_img = _config.pub_predicted_and_past_feats_img;

    this->_sensor_fps = _config.sensor_fps;
    this->_processing_factor = _config.processing_factor;
    this->_loop_period_ns = 1e9/(this->_sensor_fps/(double)this->_processing_factor);

    this->_erosion_size_detecting = _config.erosion_size_detect;
    this->_erosion_size_tracking = _config.erosion_size_track;

    this->_attention_to_motion = _config.attention_to_motion;
    this->_min_time_to_detect_motion = _config.min_time_to_detect_motion;
    this->_min_depth_difference = _config.min_depth_difference;
    this->_min_area_size_pixels = _config.min_area_size_pixels;

    OMIP_INFO_STREAM_NAMED(
...
void PointFeatureTracker::_InitializeVariables()
{
    ...
    this->_camera_info_ptr = CameraIntrinsics::Ptr(new CameraIntrinsics());
    ...
    this->_tracked_features_img = StampedImage::Ptr(new StampedImage());
    this->_tracked_features_with_pred_msk_img = StampedImage::Ptr(new StampedImage());
    this->_depth_edges_img = StampedImage::Ptr(new StampedImage());
    this->_predicting_mask_img = StampedImage::Ptr(new StampedImage());
    this->_tracking_mask_img = StampedImage::Ptr(new StampedImage());
    this->_detecting_mask_img = StampedImage::Ptr(new StampedImage());
    this->_predicted_and_previous_features_img = StampedImage::Ptr(new StampedImage());

    this->_previous_time_of_motion_detection_s = 0.0;
}
...
void PointFeatureTracker::_TrackFeatures()
{
    ...
    if(true && time_since_last_prediction < this->_loop_period_ns/2.0)
    {
        ...
        OMIP_INFO_STREAM_NAMED("PointFeatureTracker._TrackFeatures","Using " << using_pred << " predicted and " << using_prev << " previous Locations.");
    }else{

        OMIP_ERROR_STREAM_NAMED("PointFeatureTracker.measurementCallback",
    ...
    OMIP_INFO_NAMED("FeatureTracker._TrackFeatures",
...
int PointFeatureTracker::_Retrieve3DCoords(std::vector<cv::Point2f>& points_in_2d, std::vector<bool>& points_status, std::vector<cv::Point3d>& points_in_3d)
{
         ...
         features_idx++)
    {
        if (points_status[features_idx])
        {
            ...
            {
                cv::Point p = cv::Point(cvRound(points_in_2d[features_idx].x),cvRound(points_in_2d[features_idx].y));
            ...
            if (!std::isnan(x_coord) && !std::isnan(y_coord) && !std::isnan(z_coord))
            {
