 ...
 * - omip_msgs::ShapeTrackerStates -> ShapeTrackerStates.
 * - geometry_msgs::{Pose,Twist}WithCovariancePtr -> {Pose,Twist}WithCovariance::Ptr.
 * - rb_tracker/RBTrackerDynReconfConfig.h and setDynamicReconfigureValues(...)
 *   dropped entirely (no ROS-free equivalent — same precedent as
 *   feature_tracker in Phase 2): the 3 fields it controlled
 *   (cam_motion_constraint, min_feats_new_rb, min_feats_survive_rb) are
 *   already plain constructor arguments / members here, just no longer
 *   live-updatable at runtime via a ROS service.
 * - <tf/transform_listener.h> dropped: unused in this class (verified in
 *   Phase 0 — no tf:: symbol referenced).
 * - omip_msgs/RigidBodyTwistWithCovMsg.h dropped: unused in this class.
 */
...
public:
   ...
   */
     ...
     */
    virtual void processMeasurementFromShapeTracker(const ShapeTrackerStates &meas_from_st);
   ...
   */
    std::vector<PoseWithCovariance::Ptr> getPosesWithCovariance() const;
   ...
   */
    std::vector<TwistWithCovariance::Ptr> getPosesECWithCovariance() const;
   ...
   */
    std::vector<TwistWithCovariance::Ptr> getVelocitiesWithCovariance() const;
