 ...
 * Enjoy!
 *
 * Ported to a ROS-free library, per PORTING_NOTES.md Phase 3:
 ...
 * - Eigen::Twistd (lgsm) -> omip::Twistd (see LieGroup.hpp, Phase 2).
 * - rb_tracker/RBTrackerDynReconfConfig.h, ros/ros.h, tf headers, tf_conversions
 *   dropped: all verified unused/vestigial in this class in Phase 0 (no
 *   ROS type or tf:: symbol from these headers is actually referenced
 *   anywhere in RBFilter.h/.cpp — the real tf usage in this package is in
 *   StaticEnvironmentFilter, ported separately).
 * - unsupported/Eigen/{MatrixFunctions,NonLinearOptimization,NumericalDiff}
 *   dropped: verified unused (no symbol from them referenced).
 */
...
#include "omip_core/FeaturesDataBase.h"
...
public:
   ...
   */
    ...
    virtual void integrateShapeBasedPose(const TwistWithCovariance twist_refinement, double pose_time_elapsed_ns);
   ...
   */
    virtual void setPredictedState(RigidBodyPoseAndVel hypothesis);
   ...
   */
    virtual Twistd getVelocity() const;
   ...
   */
    ...
    /**
   * Get the current believed pose and its covariance (6x6) as a shared pointer to a PoseWithCovariance struct
   * @return - Shared pointer to a struct containing the believed pose and its covariance
   */
    virtual PoseWithCovariance::Ptr getPoseWithCovariance() const;

    /**
   * Get the current believed pose in exponential coordinates and its covariance (6x6) as a shared pointer to a TwistWithCovariance struct
   * @return - Shared pointer to a struct containing the believed pose and its covariance
   */
    virtual TwistWithCovariance::Ptr getPoseECWithCovariance() const;

    /**
   * Get the current believed velocity and its covariance (6x6) as a shared pointer to a TwistWithCovariance struct
   * @param hypothesis - hypothesis from which we want to get the believed twist: velocity-update or brake-event update
   * @return - Shared pointer to a struct containing the believed twist and its covariance
   */
    virtual TwistWithCovariance::Ptr getVelocityWithCovariance() const;
...
protected:
   ...
   */
    ...
    Twistd _predicted_velocity_vh;   // Predicted pose based on the velocity hypothesis/model
    Twistd _predicted_velocity_bh;   // Predicted pose based on the brake-event hypothesis/model
    Twistd _predicted_velocity_kh;   // Predicted pose based on the kinematic hypothesis/model
