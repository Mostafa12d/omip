 ...
 * - tf::Transform -> Eigen::Isometry3d (the two are mathematically
 *   equivalent rigid-transform representations; Isometry3d is Eigen's own
 *   modern, fully-supported type).
 * - ros::NodeHandle _nh and tf::TransformListener _tf_listener dropped:
 *   verified in Phase 0 that _nh is never used and _tf_listener is only
 *   referenced inside commented-out (dead) code in constrainMotion().
 * - <tf/transform_listener.h> dropped accordingly.
 */
...
public:
   ...
   */
     ...
     * @param previous_current_Tf Resulting transformation. It is the transformation of the static environment from the
     * previous to the current frame
     */
    virtual void estimateDeltaMotion(std::vector<Feature::Id>& supporting_features_ids, Eigen::Isometry3d& previous_current_Tf);
     ...
     *
     * @param computation_type Value of the type of computation (defined in omip_core/OMIPTypeDefs.h)
     */
