...
#ifndef OMIP_CORE_JOINT_TRACKER_REVOLUTEJOINTFILTER_H_
#define OMIP_CORE_JOINT_TRACKER_REVOLUTEJOINTFILTER_H_
...
#include "omip_core/joint_tracker/pdf/NonLinearRevoluteMeasurementPdf.h"
...
public:
   ...
   *
   * @return TwistWithCovariance Pose of the second rigid body in form of a twist with covariance based on the model uncertainty
   */
  virtual TwistWithCovariance getPredictedSRBPoseWithCovInSensorFrame();
 ...
 */
  virtual TwistWithCovariance getPredictedSRBDeltaPoseWithCovInSensorFrame();
 ...
 */
  virtual TwistWithCovariance getPredictedSRBVelocityWithCovInSensorFrame();

  /**
   * Return the debug geometry that shows the type and parameters of the estimated joint
   */
  virtual std::vector<DebugGeometry> getJointDebugGeometryInRRBFrame() const;
  ...
  virtual void setCovarianceDeltaMeasurementAngular(double delta_meas);
...
protected:
   ...
   * The latent variable is the angle of the rotation of the second RB wrt to the reference RB
   */
  ...
  // To unwrap the rotation angle of the revolute joint across +-PI/+-2PI boundaries
...
#endif /* OMIP_CORE_JOINT_TRACKER_REVOLUTEJOINTFILTER_H_ */
