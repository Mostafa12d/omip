 ...
 * Enjoy!
 *
 * Ported to a ROS-free library — see PORTING_NOTES.md Phase 4. ROS type
 * replacements: geometry_msgs::TwistWithCovariance -> omip::TwistWithCovariance,
 * Eigen::Twistd (lgsm) -> omip::Twistd, visualization_msgs::Marker ->
 * omip::DebugGeometry (method renamed getJointMarkersInRRBFrame() ->
 * getJointDebugGeometryInRRBFrame(), judgment call J-series). Dropped:
 * tf::TransformBroadcaster _tf_pub (confirmed dead, no sendTransform() call
 * anywhere) and the ros::NodeHandle/ros::Publisher pair plus every
 * `#ifdef PUBLISH_PREDICTED_POSE_AS_PWC` block (the macro was permanently
 * commented out in the original source — `//#define PUBLISH_PREDICTED_POSE_AS_PWC 1`
 * — so those blocks were dead code by construction; same category as prior
 * confirmed-dead-code removals, see PORTING_NOTES.md J9/J10).
 */

#ifndef OMIP_CORE_JOINT_TRACKER_JOINTFILTER_H_
#define OMIP_CORE_JOINT_TRACKER_JOINTFILTER_H_
...
public:
   ...
   *
   * @return Twistd Predicted pose-twist of the second rigid body (SRB) using the frame of the reference rigid body (RRBF) as observation and ref frame
   */
    virtual Twistd getPredictedMeasurement() const;
   ...
   */
    virtual TwistWithCovariance getPredictedSRBPoseWithCovInSensorFrame() = 0;
   ...
   * based on the predicted measurement and the predicted next pose of the reference rigid body (RRB) in the sensor frame (SF)
   *
   * @return TwistWithCovariance Change in pose of the second rigid body in form of a twist with covariance based on the model uncertainty
   */
    virtual TwistWithCovariance getPredictedSRBDeltaPoseWithCovInSensorFrame() =0;
   ...
   * based on the predicted measurement and the predicted next pose of the reference rigid body (RRB) in the sensor frame (SF)
   *
   * @return TwistWithCovariance Velocity of the second rigid body in form of a twist with covariance based on the model uncertainty
   */
    virtual TwistWithCovariance getPredictedSRBVelocityWithCovInSensorFrame() =0;
   ...
   */
    ...
    /**
   * Return the debug geometry that shows the type and parameters of the estimated joint
   */
    virtual std::vector<DebugGeometry> getJointDebugGeometryInRRBFrame() const = 0;
...
protected:
   ...
   */
    ...
    Twistd _rrb_current_pose_in_sf;
    Twistd _rrb_previous_pose_in_sf;
    Eigen::Matrix<double, 6, 6> _rrb_pose_cov_in_sf;
    Twistd _rrb_current_vel_in_sf;
    Eigen::Matrix<double, 6, 6> _rrb_vel_cov_in_sf;

    Twistd _srb_current_pose_in_sf;
    Eigen::Matrix<double, 6, 6> _srb_pose_cov_in_sf;
    Twistd _srb_current_vel_in_sf;

    // Twists relative to the reference rigid body frame (rrbf)
    Twistd _srb_initial_pose_in_rrbf;
    Eigen::Matrix<double, 6, 6> _srb_initial_pose_cov_in_rrbf;
    Twistd _srb_current_pose_in_rrbf;
    Eigen::Matrix<double, 6, 6> _srb_current_pose_cov_in_rrbf;

    Twistd _srb_previous_pose_in_rrbf;
    Twistd _srb_predicted_pose_in_rrbf;
    Twistd _srb_previous_predicted_pose_in_rrbf;
    Twistd _current_delta_pose_in_rrbf;
    Eigen::Matrix<double, 6, 6> _current_delta_pose_cov_in_rrbf;
    Twistd _previous_delta_pose_in_rrbf;
    Twistd _predicted_delta_pose_in_rrbf;
    std::vector<Twistd> _delta_poses_in_rrbf;
...
#endif /* OMIP_CORE_JOINT_TRACKER_JOINTFILTER_H_ */
