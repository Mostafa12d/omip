 ...
 * Enjoy!
 *
 * Ported to a ROS-free library, per PORTING_NOTES.md Phase 2:
 * - ros::NodeHandle _node_handle (used only to read ROS params) -> dropped;
 *   parameters now come from a FeatureTrackerConfig passed to the
 *   constructor.
 * - sensor_msgs::CameraInfo::Ptr -> CameraIntrinsics::Ptr.
 * - cv_bridge::CvImagePtr members -> StampedImage::Ptr.
 * - ros::Time members -> double (seconds/nanoseconds, matching whichever
 *   unit the original field used) using a wall-clock std::chrono helper in
 *   the .cpp where the original used ros::Time::now().
 * - message_filters/image_transport/camera_info_manager: never used inside
 *   this class in the original (only in the Node) — no trace here to drop.
 */
...
public:
   ...
   * Constructor
   */
    PointFeatureTracker(double loop_period_ns, const FeatureTrackerConfig& config, bool using_pc = false, std::string ft_ns = std::string(""));
   ...
   * @param occ_mask_img - New occlusion mask image
   */
    virtual void setOcclusionMaskImg(const cv::Mat& occ_mask_img);
   ...
   */
    virtual void setCameraInfoMsg(const CameraIntrinsics& camera_info);
   ...
   */
    ...
    virtual StampedImage::Ptr getRGBImg();

    virtual StampedImage::Ptr getDepthImg();
    ...
    */
    virtual StampedImage::Ptr getTrackedFeaturesImg();
    ...
    */
    virtual StampedImage::Ptr getTrackedFeaturesWithPredictionMaskImg();
    ...
    */
    virtual StampedImage::Ptr getDepthEdgesImg();

    /**
    * Get the image with the depth edges, used to reject Features that are on edges
    * @return - Shared pointer to the image of depth edges
    */
    virtual StampedImage::Ptr getPredictingMaskImg();
    ...
    */
    virtual StampedImage::Ptr getTrackingMaskImg();
    ...
    */
    virtual StampedImage::Ptr getDetectingMaskImg();
    ...
    */
    virtual StampedImage::Ptr getPredictedAndLastFeaturesImg();
...
protected:
    ...
    /**
   * Read parameters from the FeatureTrackerConfig
   */
   ...
   */
    ...
    FeatureTrackerConfig _config;
    CameraIntrinsics::Ptr _camera_info_ptr;

    // PCL variables
    PointCloudPCL::ConstPtr _received_point_cloud;

    // OpenCV variables
    StampedImage::Ptr _depth_img_ptr, _rgb_img_ptr, _current_bw_img_ptr, _previous_bw_img_ptr, _occlusion_msk_img_ptr;
    StampedImage::Ptr _tracked_features_img, _tracked_features_with_pred_msk_img, _depth_edges_img, _predicting_mask_img;
    StampedImage::Ptr _tracking_mask_img, _detecting_mask_img, _predicted_and_previous_features_img;
    ...
    // Wall-clock seconds (was ros::Time), used only to gate the
    // BETA "attention to motion" feature at _min_time_to_detect_motion
    // cadence — see PORTING_NOTES.md Phase 2 open question about
    // wall-clock vs. simulation-time for this gate.
    double _previous_time_of_motion_detection_s;
