 ...
 * - sensor_msgs::CameraInfo* -> const CameraIntrinsics& (only fields
 *   actually used — K, P, width, height — are kept, see types.hpp).
 * - cv_bridge::CvImagePtr -> StampedImage::Ptr (cv::Mat + encoding string +
 *   timestamp; the encoding string is preserved since several algorithms
 *   branch on it, e.g. "32FC1" vs "16UC1" depth encodings).
 * - camera_info_manager (ROS package, unused beyond the include) dropped.
 * - setDynamicReconfigureValues(...) dropped entirely: dynamic_reconfigure
 *   has no ROS-free equivalent: the (small) set of fields it controlled are
 *   now plain mutable fields on FeatureTrackerConfig, set once by the
 *   caller instead of live-toggled through a ROS service.
 */

#ifndef FEATURE_TRACKER_H_
#define FEATURE_TRACKER_H_

#include "omip_core/RecursiveEstimatorFilterInterface.h"
...
public:
    ...
    * @param occ_mask_img - Occlusion mask image
    */
    ...
    * to the image plane
    * @param camera_info - Camera intrinsics (replaces sensor_msgs::CameraInfo)
    */
    ...
    {
        return StampedImage::Ptr(new StampedImage());
    }
