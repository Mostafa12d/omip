 ...
 * Enjoy!
 *
 * Ported to a ROS-free library: this interface was already 100% ROS-free
 * in the original (see PORTING_NOTES.md, omip_common section) — unchanged
 * besides the include path. This is the base class every ported Filter
 * derives from; the companion RecursiveEstimatorNodeInterface.h (fully
 * ROS-coupled by design) is NOT ported — it is replaced by the MuJoCo
 * orchestrator per the mission brief.
 */
